...
from models.low_stock_model import LowStockProduct
import calendar
...
class Crud:
            ...
            with self.connection.conn() as conn:
                with conn.cursor() as cur:
                    ...
                    if response[0] is None:
                        return Response(
                            success=False, error="No se encontraron ventas para hoy"
                        )
                    else:
            ...
            with self.connection.conn() as conn:
                with conn.cursor() as cur:
                    ...
                    if response[0] is None:
                        return Response(
                            success=True,
                            error=f"No se encontraron ventas para la fecha {target_date}",
                            data=0,
                        )
                    else:
            ...
            with self.connection.conn() as conn:
                with conn.cursor() as cur:
                    ...
                    if response[0] is None:
                        return Response(
                            success=False,
                            error=f"No se encontraron ventas entre {first_day_of_month} y {last_day_of_month}",
                        )
                    else:
        ...
        try:
            with self.connection.conn() as conn:
                with conn.cursor() as cur:
                    ...
                    if not response:
                        return Response(
                            success=False, error="No se encontraron productos vendidos"
                        )
                    else:
            ...
            with self.connection.conn() as conn:
                with conn.cursor() as cur:
                    ...
                    if response[0] is None:
                        return Response(
                            success=False,
                            error="No se encontraron productos en inventario",
                        )
                    else:
    ...
    def get_products_in_low_stock(self):
        try:
            with self.connection.conn() as conn:
                with conn.cursor() as cur:
                    ...
                    if not response:
                        ...
                        return Response(
                            success=True,
                            data=[
                                LowStockProduct(id=row[2], name=row[0], stock=row[1])
                                for row in response
                            ],
                        )
        except Exception as e:
